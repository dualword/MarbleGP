...
namespace dustbin {
  namespace gui {
    ...
    void CTrackButton::setTexture(const std::string& a_sTexture) {
      m_sImage = a_sTexture;
      m_pImage = CGlobal::getInstance()->createTexture(a_sTexture);
    }

    void CTrackButton::setTrack(const std::string& a_sTrack) {
      m_sTrack = a_sTrack;
    }

    const std::string& CTrackButton::getTrack() {
      return m_sTrack;
    }
    ...
    bool CTrackButton::OnEvent(const irr::SEvent& a_cEvent) {
      ...
      if (a_cEvent.EventType == irr::EET_GUI_EVENT) {
        ...
        if (a_cEvent.MouseInput.Event == irr::EMIE_LMOUSE_PRESSED_DOWN) {
          ...
          if (m_bLDown && m_bHovered && Parent != nullptr) {
            irr::SEvent l_cEvent;

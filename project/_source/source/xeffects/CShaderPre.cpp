...
CShaderPreprocessor::CShaderPreprocessor(irr::video::IVideoDriver* driverIn, irr::IrrlichtDevice *a_pDevice) : driver(driverIn) 
{m_pDevice = a_pDevice; initDefineMap();};
...
std::string getFileContent(const std::string pFile, irr::IrrlichtDevice *a_pDevice)
{
  /*
  irr::io::IFileSystem *l_pFs = a_pDevice->getFileSystem();

  irr::io::IReadFile *l_pFile = l_pFs->createAndOpenFile(pFile.c_str());
  irr::c8 l_sContent[0xFFFF];

  l_pFile->read(l_sContent, 0xFFFF);

  std::string Content = l_sContent;

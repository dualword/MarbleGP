...
AmbientColour(0x0), use32BitDepth(use32BitDepthBuffers)
{
	...
	CShaderPreprocessor sPP(driver, device);
			   ...
			   (driver->getDriverType() == EDT_DIRECT3D9 && driver->queryFeature(EVDF_PIXEL_SHADER_2_0))))
	{
		...
		Depth = gpu->addHighLevelShaderMaterial(
			sPP.ppShader(SHADOW_PASS_1V).c_str(), "vertexMain", video::EVST_VS_2_0,
			sPP.ppShader(SHADOW_PASS_1P).c_str(), "pixelMain", video::EPST_PS_2_0,
			depthMC, video::EMT_SOLID);

		DepthT = gpu->addHighLevelShaderMaterial(
			sPP.ppShader(SHADOW_PASS_1V).c_str(), "vertexMain", video::EVST_VS_2_0,
			sPP.ppShader(SHADOW_PASS_1PT).c_str(), "pixelMain", video::EPST_PS_2_0,
			depthMC, video::EMT_TRANSPARENT_ALPHA_CHANNEL_REF);

		WhiteWash = gpu->addHighLevelShaderMaterial(
			sPP.ppShader(SHADOW_PASS_1V).c_str(), "vertexMain", video::EVST_VS_2_0,
			sPP.ppShader(WHITE_WASH_P).c_str(), "pixelMain", video::EPST_PS_2_0,
			depthMC, video::EMT_SOLID);
		...
		WhiteWashTAdd = gpu->addHighLevelShaderMaterial(
			sPP.ppShader(SHADOW_PASS_1V).c_str(), "vertexMain", video::EVST_VS_2_0,
			sPP.ppShader(WHITE_WASH_P_ADD).c_str(), "pixelMain", video::EPST_PS_2_0,
		...
		for(u32 i = 0;i < EFT_COUNT;i++)
		{
			...
			Shadow[i] = gpu->addHighLevelShaderMaterial(
				sPP.ppShader(SHADOW_PASS_2V).c_str(), "vertexMain", vertexProfile,
				sPP.ppShader(SHADOW_PASS_2P).c_str(), "pixelMain", pixelProfile,
		...
		LightModulate = gpu->addHighLevelShaderMaterial(
			sPP.ppShader(SCREEN_QUAD_V).c_str(), "vertexMain", vertexProfile,
			sPP.ppShader(LIGHT_MODULATE_P).c_str(), "pixelMain", pixelProfile, SQCB);

		// Simple present.
		Simple = gpu->addHighLevelShaderMaterial(
			sPP.ppShader(SCREEN_QUAD_V).c_str(), "vertexMain", vertexProfile,
			sPP.ppShader(SIMPLE_P).c_str(), "pixelMain", pixelProfile, SQCB,
			video::EMT_TRANSPARENT_ADD_COLOR);

		// VSM blur.
		VSMBlurH = gpu->addHighLevelShaderMaterial(
			sPP.ppShader(SCREEN_QUAD_V).c_str(), "vertexMain", vertexProfile,
			sPP.ppShader(VSM_BLUR_P).c_str(), "pixelMain", pixelProfile, SQCB);
	...
	irr::video::E_MATERIAL_TYPE baseMaterial)
{
	...
	s32 PostMat = gpu->addHighLevelShaderMaterial(
		sPP.ppShader(SCREEN_QUAD_V).c_str(), "vertexMain", VertexLevel,

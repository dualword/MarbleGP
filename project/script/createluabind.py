...
def CreateTables(a_Json):
  l_Tables    = a_Json["tables"]
  l_OutHeader = g_IncludeOutput + "/" + g_Prefix + "tables.h"
  l_OutSource = g_SourceOutput  + "/" + g_Prefix + "tables.cpp";
  ...
  l_Header.write("#include <" + g_IncludePath + "/" + g_Prefix + "enums.h>\n")
...
def CreateEnums(a_Json):
  ...
  l_OutHeader = g_IncludeOutput + "/" + g_Prefix + "enums.h"
  l_OutSource = g_SourceOutput  + "/" + g_Prefix + "enums.cpp";
  ...
  l_Source.write("/** This file was generated by the Dustbin::Games LUAWrapGenerator (w) 2020 by Christian Keimel **/\n\n")
  l_Source.write("#include <" + g_IncludePath + "/" + g_Prefix + "enums.h>\n")
  l_Source.write("\n")
  ...
  for l_Value in l_Enums:
    ...
    for l_Item in l_Enums[l_Value]:
      l_Source.write("      lua_pushinteger(a_pState, (int)" + l_Item["name"] + ");");
      if "comment" in l_Item:
        l_Source.write("// " + l_Item["comment"] + "\n")
      else:
        l_Source.write("\n")
...
def CreateHeader(a_FilePath, a_Name, a_Class, a_Type, a_Json):
  ...
  if "inherits" in a_Class and a_Class["inherits"] != "":
    ...
    l_Header.write(" : public lua::ILuaClass")
  ...
  if a_Type == "singleton":
    ...
    for l_Member in a_Class["members"]:
      if "constructor" in l_Member and l_Member["constructor"] == "true":
        l_Header.write(", " + l_Member["type"] + " a_" + l_Member["name"])
...
def CreateClassBinding(a_Source, a_Name, a_Class, a_Type, a_Json):
  ...
  if "methods" in a_Class:
    ...
    if a_Type == "singleton":
      ...
      for l_Member in a_Class["members"]:
        if "constructor" in l_Member and l_Member["constructor"] == "true":
          a_Source.write(", " + l_Member["type"] + " a_" + l_Member["name"])
          
          if l_MemberInit == "":
            l_MemberInit = " : "
          else:
            l_MemberInit = l_MemberInit + ", "
          
          l_MemberInit = l_MemberInit + "m_" + l_Member["name"] + "(a_" + l_Member["name"] + ")"
      
      a_Source.write(") " + l_MemberInit + " {\n")
      ...
      if "singletons" in a_Class:
        ...
        for l_Singleton in a_Class["singletons"]:
          ...
          for l_SgtMember in a_Json["singletons"][l_Singleton]["members"]:
            if "constructor" in l_SgtMember and l_SgtMember["constructor"] == "true":
              a_Source.write(", m_" + l_SgtMember["name"])
      ...
      a_Source.write("          printf(\"LUA Error: \\\"%s\\\"\\n\", l_sMsg.c_str());\n")
      a_Source.write("        }\n")
      a_Source.write("        else {\n")
...
def CreateBinding(a_Json):
  l_Name = g_SourceOutput + "/" + g_Prefix + "binding.cpp"
...
if len(sys.argv) > 3:
  g_Namespace = sys.argv[3]

g_Definitions = json.load(open(g_Json))

if "folders" in g_Definitions:
  print("Folders found in JSON.")
  if "header" in g_Definitions["folders"]:
    print("  Write headers to " + g_Definitions["folders"]["header"])
    g_IncludeOutput = g_Definitions["folders"]["header"]
    
  if "source" in g_Definitions["folders"]:
    print("  Write sources to " + g_Definitions["folders"]["source"])
    g_SourceOutput = g_Definitions["folders"]["source"]
    
  if "include" in g_Definitions["folders"]:
    print("  Include path " + g_Definitions["folders"]["include"])
    g_IncludePath = g_Definitions["folders"]["include"]

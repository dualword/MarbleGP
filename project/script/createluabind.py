...
def CreateTables(a_Json):
  ...
  l_Header.write("#include <luawrap.hpp>\n")
...
def CreateHeader(a_FilePath, a_Name, a_Class, a_Type, a_Json):
  ...
  l_Header.write("#ifdef _IRRLICHT_HEADER_NOSUB\n")
...
def CreateClassBinding(a_Source, a_Name, a_Class, a_Type, a_Json):
  ...
  if "methods" in a_Class:
    if "lua_from" in a_Class["methods"]:
      for l_Name in a_Class["methods"]["lua_from"]:
        ...
        if a_Type == "singleton":
          ...
          a_Source.write("  }\n")
          a_Source.write("  else throw std::string(\"\\\"" + a_Name + "\\\" instance not found on lua heap.\");\n")
        else:
        ...
        a_Source.write("  }\n")
        a_Source.write("  catch (std::string &s) {\n")
        a_Source.write("    luaL_error(a_pState, (std::string(\"Function " + a_Name + ":" + l_Name + " failed: \\\"\") + std::string(s + std::string(\"\\\"\"))).c_str());\n")
        a_Source.write("  }\n")
    ...
    if "lua_to" in a_Class["methods"]:
      for l_Name in a_Class["methods"]["lua_to"]:
        ...
        if "comment" in l_Method:
          ...
          a_Source.write("      throw std::string(l_sMessage.c_str());\n")
          a_Source.write("    }\n")
    ...
    if a_Type == "singleton":
      ...
      a_Source.write("      throw std::string(l_sMsg.c_str());\n")
      a_Source.write("    }\n")
      a_Source.write("    else {\n")
...
def CreateBinding(a_Json):
  ...
  l_Source.write("#include <lua/CLuaTypeHelpers.h>\n")
  l_Source.write("\n")
